...
class SetSensorParams:
    ...
    def to_bytes(self) -> bytes:
        return b''.join(struct.pack('<iB', a, b) for a, b in self.sensor_paramss) # little endian 4B 1B
...
class SensorSamples:
    ...
    @classmethod
    def list_from_bytes_with_remainder(cls, _bytes:bytes, expected_sizes:tuple[int, ...]) -> tuple[list['Self'], bytes]:
        i = 0
        cls_list:list[cls] = []
