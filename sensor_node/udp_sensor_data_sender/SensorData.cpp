...
uint32_t SensorData::write(double value) {
  if (currentIndex + sizeof(uint32_t) + sizeof(double) <= BUFFER_SIZE) {
    // Přidání časového razítka (millis()) - uložené jako 4 bajty (uint32_t)
    uint32_t timestamp_millis = millis();  // Získání aktuálního času v ms
    memcpy(&buffer[currentIndex], &timestamp_millis, sizeof(uint32_t));
